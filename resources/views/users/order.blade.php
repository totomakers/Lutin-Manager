        ...
        <div class="animated fadeIn">
            <div class="row-fluid">
                <div class="col-xs-offset-4 col-xs-4">
                    ...
                    <ul class="list-group">
                        <li class="list-group-item"><strong>Numéro de commande :</strong> <span
                                    class="pull-right">{{ $order->id }}</span></li>
                        <li class="list-group-item"><strong>Nom :</strong> <span
                                    class="pull-right">{{ $order->name }}</span></li>
                        <li class="list-group-item"><strong>Adresse :</strong> <span
                                    class="pull-right">{{ $order->address }}</span></li>
                    </ul>
                    </br>
                </div>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" min="0" max="{{ $row->quantity }}"
                                               name="{{$row->item->name}}" class="form-control form_number input-sm"
                                               id="item-quantity-{{ $row->item->id }}">
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
...
@section('js_page')
    <script>
        ...
        var items_quantity = [@foreach($order->rows as $row) {
            'id': '{{$row->item->id}}',
